...
def _load():
    ...
    return inst, tw.register_from_instance(inst), inst["graph"]


def _subreg(reg, survivors):
    return tw.Register(
        coords_um=reg.coords_um[list(survivors)],
...
def _induced_edges(reg, survivors):
    return tw.build_graph_from_register(_subreg(reg, survivors)).number_of_edges()
...
def _init_state(inst, G):
    ...
    res = tw.load_results(HEADLINE)
    sel, _r, _t, _d = tw.decode_shots(res["shots"], inst["n_atoms"], policy="postselect")
    mis1, _v, _n = tw.best_valid_set(sel, G)
...
def _device():
    ...
    return QbraidProvider().get_device(tw.AQUILA_DEVICE_ID)
...
def _submit_next(s, reg, shots):
    ...
    tw.assert_valid_register(sub)
    cost = tw.task_credits(shots)
    print(
        f"channel {len(s['rounds'])+1}: submitting {sub.n}-atom residual, {shots} shots "
        f"(cost {cost} credits) ..."
    )
    job = _device().run(tw.build_ahs_program(sub), shots=shots)  # <-- the only QPU call
...
def _collect_pending(s, reg, wait=True):
    ...
    while True:
        ...
        if any(b in st.upper() for b in ("FAIL", "CANCEL", "ERROR")):
            print(
                f"  channel {len(s['rounds'])+1} job ended {st} — transient device failure; "
    ...
    shots = [tw.Shot(m.pre_sequence, m.post_sequence) for m in meas]
    Gsub = tw.build_graph_from_register(_subreg(reg, survivors))
    sel, ret, tot, _d = tw.decode_shots(shots, len(survivors), policy="postselect")
    sub_mis, viol, nvalid = tw.best_valid_set(sel, Gsub)
    nodes = sorted(int(survivors[i]) for i in sub_mis)  # map sub-index -> original
    s["rounds"].append(
        {
            "channel": len(s["rounds"]),
            "source": "QPU (Aquila, measured)",
            "task_id": tid,
            "shots": len(shots),
            "size": len(nodes),
            "nodes": nodes,
            "blockade_rejected": int(viol),
            "valid_shots": int(nvalid),
        }
    )
    ...
    _save_state(s)
    print(
        f"  channel {len(s['rounds'])}: {len(nodes)} towers "
...
def _finish_edgeless(s, reg):
    ...
    if survivors and _induced_edges(reg, survivors) == 0:
        s["rounds"].append(
            {
                "channel": len(s["rounds"]),
                "source": "trivial (edgeless residual)",
                "task_id": None,
                "shots": 0,
                "size": len(survivors),
        ...
        _save_state(s)
        print(
            f"final channel {len(s['rounds'])}: {len(survivors)} towers "
    ...
    while s["survivors"]:
        ...
        while True:  # retry this channel on transient device failure
            ...
            if outcome == "failed":
                ...
                print(
                    f"  resubmitting channel {len(s['rounds'])+1} (attempt {attempt+1}/{max_retries+1}) ..."
                )
                time.sleep(5)
...
def assemble():
    ...
    for r in s["rounds"]:
        extra = (
            ""
            if r["source"].startswith("trivial")
            else f"  task {r['task_id'][-12:]}  ({r.get('valid_shots','-')} valid shots)"
        )
    ...
    out = {
        ...
        "rounds": s["rounds"],
        "valid": bool(ok) and covered == inst["n_atoms"],
...
def display():
    ...
    ax.set_title(
        f"Aquila channel plan — {d['n_channels']} channels (all measured on QPU)",
        color="white",
        loc="left",
        fontsize=12,
    )
    ...
    [sp.set_color("#33404f") for sp in ax.spines.values()]
    fig.tight_layout()
...
def plan(shots):
    ...
    print(
        f"rounds completed: {done}   survivors: {len(s['survivors'])}   pending: {bool(s['pending'])}"
    )
    if not s["survivors"]:
        print("all towers assigned — run --assemble.")
        return
    e = _induced_edges(reg, s["survivors"])
    if e == 0:
        print(
            f"next: edgeless residual ({len(s['survivors'])} towers) -> one final channel, NO QPU cost."
        )
    else:
        sub = _subreg(reg, s["survivors"])
        tw.assert_valid_register(sub)
        print(
            f"next channel {done+1}: submit {sub.n}-atom residual ({e} conflict edges), "
            f"{shots} shots -> cost {tw.task_credits(shots)} credits. Register is Aquila-LEGAL."
        )
...
def main():
    ...
    g.add_argument(
        "--run-all",
        action="store_true",
        help="submit/collect every remaining channel (spends credits)",
    )
