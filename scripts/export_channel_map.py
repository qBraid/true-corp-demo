...
def export(assign_path, scale=1.0, legend=True):
    ...
    palette = sw.CHANNEL_PALETTE

    uri, W, H, ext, attr = sw._basemap(BP, BJ)
    px, py = sw._project(coords[:, 0], coords[:, 1], ext, W, H)
    ...
    for c, nodes in enumerate(channels):
        ...
        fig.add_trace(go.Scatter(x=px[nodes], y=py[nodes], mode="markers", hoverinfo="skip",
                                 showlegend=False, marker=dict(size=52, color=col, opacity=0.16)))
        fig.add_trace(go.Scatter(x=px[nodes], y=py[nodes], mode="markers", hoverinfo="skip",
                                 name=f"channel {c+1}  ({len(nodes)})", showlegend=legend,
    ...
    fig.add_trace(go.Scatter(x=bx, y=by, mode="markers+text", text=names, showlegend=False,
    ...
    if legend:
        fig.update_layout(showlegend=True, legend=dict(
            ...
            bgcolor="rgba(11,15,20,0.78)", bordercolor="#33404f", borderwidth=1,
            font=dict(color=sw.TEXT, size=22, family=sw.FONT),
            itemsizing="constant", tracegroupgap=2))

    fig.write_image(OUT, width=W, height=H, scale=scale)
    n_ch = len(channels)
    print(f"saved -> {OUT}   ({int(W*scale)} x {int(H*scale)} px, {n_ch} channels, "
