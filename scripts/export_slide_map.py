...
def export(show_graph=True, scale=1.0):
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            hoverinfo="skip",
            marker=dict(size=24, color=QB_PURPLE_BRIGHT, line=dict(color="white", width=1.3)),
        )
    )
    ...
    print(
        f"saved -> {OUT}   ({int(W*scale)} x {int(H*scale)} px, "
