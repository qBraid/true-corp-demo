...
Channel 1 is the executed QuEra Aquila result (the 34-cell sleep/independent set);
...
def _qpu_channel_one(inst):
    ...
    res = sc.load_results(QPU)
    sel, _ret, _tot, _defect = sc.decode_shots(res["shots"], inst["n_atoms"], policy="postselect")
    best, _viol, _nvalid = sc.best_valid_set(sel, inst["graph"])
    return best
...
def run():
    ...
    print(f"overspend      : {overspend}  " +
          ("(greedy is optimal here)" if overspend == 0 else
           f"(greedy used {overspend} channel(s) more than necessary)"))
    print(f"round sizes    : {[len(c) for c in channels]}")
    for p in prov:
        print(f"  channel {p['channel']+1}: {p['size']:2d} towers  [{p['source']:>22s}]  "
              f"(residual before: {p['residual_nodes']}n / {p['residual_edges']}e)")
    print(f"valid colouring: {'YES — every tower one channel, no conflict shares a frequency' if ok else problems}")
...
def display(inst, channels, channel_of):
    ...
    ax.set_title(f"Frequency-reuse plan — {len(channels)} channels, "
    ...
    ax.tick_params(colors="#8a94a3"); [s.set_color("#33404f") for s in ax.spines.values()]
    fig.tight_layout()
