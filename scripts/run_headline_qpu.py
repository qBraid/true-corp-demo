...
def _load_instance():
    ...
    reg = tw.register_from_instance(inst)
    G = inst["graph"]
...
def _build_and_validate(shots):
    ...
    tw.assert_valid_register(reg)  # loud failure if not Aquila-legal
    ...
    print(
        f"register   : {span[0]:.1f} x {span[1]:.1f} um  "
        f"(Aquila field {tw.FIELD_UM} um)  ->  LEGAL"
    )
    print(
        f"shots      : {shots}   cost = {tw.task_credits(shots)} credits "
        f"({tw.CREDITS_PER_TASK} + {shots}x{tw.CREDITS_PER_SHOT})"
    )
...
def _summarise(inst, G, exact):
    ...
    loaded = tw.load_results(OUT_RESULTS)
    sel, ret, tot, defect = tw.decode_shots(loaded["shots"], inst["n_atoms"], policy="postselect")
    best, viol, nvalid = tw.best_valid_set(sel, G)
    ok = tw.verify_independent_set(G, best)
    ...
    print(
        f"best set   : |S| = {len(best)} of {inst['n_atoms']}  "
...
def dry_run(shots):
    ...
    try:
        ...
        dev = QbraidProvider().get_device(tw.AQUILA_DEVICE_ID)
        print(f"device     : {tw.AQUILA_DEVICE_ID}   status = {dev.status()}")
        print(f"pricing    : {dev.metadata().get('pricing')}")
    except Exception as e:
        print(
            f"device     : not resolvable here ({repr(e)[:70]}) — offline validation still passed"
        )
...
def submit(shots):
    ...
    with open(TASK_FILE, "w") as f:
        json.dump(
            {"task_id": task_id, "shots": shots, "instance": os.path.basename(INSTANCE)},
            f,
            indent=2,
        )
...
def collect(task_id):
    ...
    ]  # qBraid Result -> AnalogResultData
    tw.save_results(
        OUT_RESULTS,
        instance_name="sukhumvit_90",
